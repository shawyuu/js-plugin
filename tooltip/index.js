	...
	if (!isVisible) {
		...
		if (_p.startsWith("top")) {
			if (rect.top >= 0 && rect.left < 0) {
				...
				/*The condition will not be enforced.(不会执行这个条件)*/
				replaceObj[_p] = "top";
			}
...
function setPosition(elWrapper, _p, _target) {
	...
	let w = _target.width,
		...
		left = _target.left,
		top = _target.top;
	let ew = toolTipRect.width,
		eh = toolTipRect.height;
	if (ew > 200 && sw <= 576) {
	...
	let gap = options.showArrow ? 10 : 6;
	if (_p.startsWith('top')) {
...
function closeTooltip() {
	...
	if (!_el) return
	_el.style.animation = `toolTipAnimateOut${options.animation} 0.3s ease-in`;
